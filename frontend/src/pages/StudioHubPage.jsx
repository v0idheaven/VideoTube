...
const StudioHubPage = () => {
  ...
  const analyticsBars = [42, 56, 48, 64, 58, 51, 72, 79, 74, 86, 83, 94];
      ...
      <section className="space-y-6">
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr),340px]">
          <div className="rounded-[28px] border border-white/10 bg-[#181818] p-6">
            <p className="text-xs font-semibold uppercase tracking-[0.28em] text-white/35">Analytics preview</p>
            <h2 className="mt-3 text-2xl font-semibold tracking-[-0.04em] text-white">
              Recent momentum
            </h2>
            <p className="mt-2 text-sm leading-7 text-white/48">
              A quick visual pulse of the channel while your main dashboard stats come from the real backend totals.
            </p>
            <div className="mt-8 flex h-40 items-end gap-2">
              {analyticsBars.map((barHeight, index) => (
                <div className="flex-1" key={index}>
                  <div
                    className="w-full rounded-t-[10px] bg-[#ff2d2d]/85 transition hover:bg-[#ff2d2d]"
                    style={{ height: `${barHeight}%` }}
                  />
                </div>
              ))}
            </div>
            <div className="mt-4 flex justify-between text-[11px] uppercase tracking-[0.2em] text-white/28">
              <span>Mar 1</span>
              <span>Mar 14</span>
              <span>Mar 30</span>
            </div>
        ...
        <div className="rounded-[28px] border border-white/10 bg-[#181818] p-6">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm leading-7 text-white/48">
                Manage draft and published videos from the same dashboard feed powered by `/api/v1/dashboard/videos`.
              </p>
            </div>
